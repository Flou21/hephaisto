...
namespace Watchtower.ServiceDefaults;
...
/// so in the environment Watchtower actually lives in it would be blind. For a project whose
        ...
        builder.Services
            ...
            .WithMetrics(metrics =>
            {
                metrics
                    ...
                    .AddMeter(WatchtowerTelemetry.MeterName)
                    ...
                    .AddMeter(WatchtowerTelemetry.ExtensionsAiSourceName);
            })
            .WithTracing(tracing =>
            {
                tracing
                    .AddSource(WatchtowerTelemetry.ActivitySourceName)
                    // gen_ai.* spans - model, prompt, tool arguments, token counts - come
                    // from here for free via ChatClientBuilder.UseOpenTelemetry().
                    .AddSource(WatchtowerTelemetry.ExtensionsAiSourceName)
