...
// IAgentModeStore, IncidentSearch, LlmBudgetService and HephaistoDbContext out of a
...
app.MapHephaistoEndpoints();  // /webhooks/*, /api/incidents/*, /api/status, /api/evidence-blobs/*
...
|---|---|---|
| `ISignalSink` | `Web/SignalSink.cs` | **Ingest.** Register the real one with `services.AddSingleton<ISignalSink, …>()` *before* `AddHephaistoWeb()`, or replace it after — `AddHephaistoWeb` uses `TryAdd`, so it never overwrites an existing registration. The placeholder logs and drops. |
...
`IncidentQueries` issues those two LINQ queries against `HephaistoDbContext` directly. It is
