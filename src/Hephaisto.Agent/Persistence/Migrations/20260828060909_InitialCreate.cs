    ...
    public partial class InitialCreate : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.InsertData(
                ...
                values: new object[] { "singleton", new DateTimeOffset(new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified), new TimeSpan(0, 0, 0, 0, 0)), "hephaisto/system", null, null, "Observe", false });
            ...
            // ----------------------------------------------------------------------
            // The application connects as hephaisto_app. Immutability of audit_events is
            ...
            // row - and HephaistoDbContext.SaveChangesAsync throws on a Modified or
            ...
            //
            // Wrapped in a DO block for exactly that reason: with no hephaisto_app role
            ...
            migrationBuilder.Sql("""
                DO $$
                BEGIN
                    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'hephaisto_app') THEN
                        GRANT USAGE ON SCHEMA public TO hephaisto_app;
                        GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO hephaisto_app;

                        REVOKE UPDATE, DELETE, TRUNCATE ON audit_events FROM hephaisto_app;
                    ELSE
                        RAISE NOTICE
                            'role hephaisto_app not present: audit_events immutability is enforced only by HephaistoDbContext in this database';
                    END IF;
                END
                $$;
                """);
