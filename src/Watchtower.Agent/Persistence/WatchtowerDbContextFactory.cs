...
/// </remarks>
public sealed class WatchtowerDbContextFactory : IDesignTimeDbContextFactory<WatchtowerDbContext>
{
    private const string LocalDefault =
        "Host=localhost;Port=5432;Database=watchtower;Username=watchtower_app;Password=watchtower";

    public WatchtowerDbContext CreateDbContext(string[] args)
    {
        var connectionString =
            Environment.GetEnvironmentVariable("ConnectionStrings__watchtower")
            ?? Environment.GetEnvironmentVariable("WATCHTOWER_DB")
            ?? LocalDefault;
        ...
        var options = new DbContextOptionsBuilder<WatchtowerDbContext>()
            ...
            {
                o.UseVector();
                o.MigrationsAssembly(typeof(WatchtowerDbContextFactory).Assembly.FullName);
            })
            .Options;
