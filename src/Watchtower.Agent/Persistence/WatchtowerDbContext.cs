...
/// <summary>
/// The only place in Watchtower that knows anything about EF Core. The domain types in
/// <c>Watchtower.Core.Domain</c> are mapped directly - there is no separate set of
...
/// </summary>
public sealed class WatchtowerDbContext(DbContextOptions<WatchtowerDbContext> options)
    : DbContext(options)
{
    /// <summary>
    ...
    private static void ConfigureDigests(ModelBuilder b)
    {
        b.Entity<IncidentDigest>(e =>
        {
            ...
            // embedding as a plain array so Watchtower.Core needs no Pgvector reference;
    ...
    /// <summary>
    /// Defence in depth. Postgres is the real enforcement - watchtower_app holds INSERT but
