...
{
    /// <summary>
    /// The mapping is a switch over alertname, so a rule that predates Watchtower still lands
    /// somewhere useful. <c>watchtower_kind</c> is the explicit override for rules written
    ...
    internal static Signal ToSignal(AlertmanagerAlert alert, AlertmanagerWebhook payload)
    {
        ...
        {
            signal.Labels["watchtower_alertmanager_url"] = payload.ExternalUrl;
        }

        if (!string.IsNullOrEmpty(alert.GeneratorUrl))
        {
            signal.Labels["watchtower_generator_url"] = alert.GeneratorUrl;
        }
