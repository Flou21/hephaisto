...
/// <see cref="WatchtowerDbContext"/> into a page would therefore pin one connection and one
    ...
    IClock clock)
{
    ...
    public async Task<IReadOnlyList<IncidentListItem>> ListAsync(IncidentListQuery query, CancellationToken ct)
    {
        ...
        else if (query.OpenOnly)
        {
            ...
            incidents = incidents.Where(i => WatchtowerDbContext.OpenStates.Contains(i.State));
        }
    ...
    public async Task<AgentStatusView> GetStatusAsync(CancellationToken ct)
    {
        ...
        var open = await db.Incidents
            .AsNoTracking()
            .CountAsync(i => WatchtowerDbContext.OpenStates.Contains(i.State), ct);
