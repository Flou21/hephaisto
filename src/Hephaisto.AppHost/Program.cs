...
// infra/app/hephaisto.yaml, applied by Tilt. This file exists so `dotnet run` from a
...
var postgres = builder
    ...
    .WithDataVolume("hephaisto-pgdata")
    .WithLifetime(ContainerLifetime.Persistent);

var db = postgres.AddDatabase("hephaisto");

var agent = builder
    .AddProject<Projects.Hephaisto_Agent>("hephaisto")
    .WithReference(db)
    .WaitFor(db)
    ...
    .WithEnvironment("HEPHAISTO_MODE", "observe");

builder
    .AddProject<Projects.Hephaisto_Simulator>("simulator")
    .WithReference(agent)
    .WaitFor(agent);
