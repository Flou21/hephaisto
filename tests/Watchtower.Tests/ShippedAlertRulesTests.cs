...
/// <c>watchtower_kind</c> labels were values like <c>PvcFillingUp</c> and
...
public class ShippedAlertRulesTests
{
    ...
    private static readonly Regex KindLine = new(@"^\s*watchtower_kind:\s*(\S+)", RegexOptions.Compiled);
    ...
    public void Every_shipped_alert_classifies_to_a_real_kind(string file, string alertName, string kindLabel)
    {
        Enum.TryParse<SignalKind>(kindLabel, ignoreCase: true, out var declared)
            .Should().BeTrue(
                $"{file}: '{alertName}' declares watchtower_kind '{kindLabel}', which is not a SignalKind "
    ...
    private static string AlertsDirectory()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "Watchtower.slnx")))
            dir = dir.Parent;
