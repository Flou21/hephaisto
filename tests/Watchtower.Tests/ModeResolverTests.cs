...
/// operator who sets <c>WATCHTOWER_MODE=observe</c> to STOP an agent running in Auto gets no
...
public class ModeResolverTests
{
    ...
    /// <c>WATCHTOWER_MODE=obserev</c> would not restrict the agent - it would REMOVE the
    ...
    {
        var resolved = ModeResolver.Resolve(
            ModeArm.Unreadable(Cfg, "/etc/watchtower/mode does not exist"),
            ModeArm.Declaring(Db, AgentMode.Auto));
